...
void TaskBar::updateLayout(int &size_w, int &size_h) {
    ...
    wlist.append(nw);
#endif
    nw = (LayoutInfo){ fShowDesktop, true, 0, true, 0, 0, taskButtonHeight ? false : true };
    wlist.append(nw);
#ifdef CONFIG_WINMENU
    nw = (LayoutInfo){ fWinList, true, 0, true, 0, 0, taskButtonHeight ? false : true};
    wlist.append(nw);
#endif
#ifndef NO_CONFIGURE_MENUS
    nw = (LayoutInfo){ fObjectBar, true, 1, true, 4, 0, taskButtonHeight ? false : true };
    wlist.append(nw);
#endif
    nw = (LayoutInfo){ fWorkspaces, taskBarWorkspacesLeft, taskBarDoubleHeight && taskBarWorkspacesTop, true, 4, 4, taskButtonHeight ? false : true };
    wlist.append(nw);

    nw = (LayoutInfo){ fCollapseButton, false, 0, true, 0, 2, taskButtonHeight ? false : true };
    wlist.append(nw);
#ifdef CONFIG_APPLET_CLOCK
    ...
    wlist.append(nw);
#ifdef CONFIG_TRAY
    nw = (LayoutInfo){ fWindowTray, false, 0, true, 1, 1, taskButtonHeight ? false : true };
    wlist.append(nw);
#endif
...
>>>>>>> refs/heads/icewm-1-3-BRANCH*/
            hh = taskButtonHeight ? taskButtonHeight : wlist[i].w->height();
            yy = y[wlist[i].row] + (h[wlist[i].row] - wlist[i].w->height()) / 2;
        }
